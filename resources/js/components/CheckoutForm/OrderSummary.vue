<template>
  <div class="mt-4 p-4 border rounded bg-gray-50">
    ...
    <div v-for="item in itemsWithDetails" :key="item.product_id" class="mb-2">
      <div class="flex justify-between">
        <span>{{ item.product_name }} x{{ item.quantity }}</span>
        <span>{{ item.baseTotal.toFixed(2) }} ৳</span>
      </div>

      <div v-if="item.variant_name" class="flex justify-between ml-4 text-gray-600">
...
const itemsWithDetails = computed(() => {
  return props.items.map((item) => {
    ...
    const variant =
      item.variant_index !== null && product?.variations?.[item.variant_index!]
    ...
    return {
      ...
      extraTotal: variant ? Number(variant.extra_price) * item.quantity : 0,
    };
  });
});
...
const total = computed(() =>
  Math.max(0, subtotal.value + (props.deliveryFee || 0) - (props.discount || 0)),
);
</script>
